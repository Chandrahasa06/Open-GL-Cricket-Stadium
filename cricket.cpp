    ...
    srand(42);
    {
        ...
        for (int y = 0; y < H; y++)
            for (int x = 0; x < W; x++) {
                ...
                pixels[idx + 0] = (unsigned char)(r < 0 ? 0 : r > 255 ? 255
                                                                      : r);
                pixels[idx + 1] = (unsigned char)(g < 0 ? 0 : g > 255 ? 255
                                                                      : g);
                pixels[idx + 2] = (unsigned char)(b < 0 ? 0 : b > 255 ? 255
                                                                      : b);
            }
...
void drawTexturedGround() {
    ...
    for (int i = 0; i < segments; i++) {
        ...
        glBegin(GL_QUADS);
        glTexCoord2f(x1i * scale + 0.5f, z1i * scale + 0.5f);
        glVertex3f(x1i, 0, z1i);
        glTexCoord2f(x2i * scale + 0.5f, z2i * scale + 0.5f);
        glVertex3f(x2i, 0, z2i);
        glTexCoord2f(x2o * scale + 0.5f, z2o * scale + 0.5f);
        glVertex3f(x2o, 0, z2o);
        glTexCoord2f(x1o * scale + 0.5f, z1o * scale + 0.5f);
    ...
    for (int i = 0; i < segments; i++) {
        ...
        glTexCoord2f(x1 * scale + 0.5f, z1 * scale + 0.5f);
        glVertex3f(x1, 0.01f, z1);
        glTexCoord2f(x2 * scale + 0.5f, z2 * scale + 0.5f);
...
void drawTexturedWall() {
    int slices = 100;
    float radius = 25, wallHeight = 8, texRepeatS = 8, texRepeatT = 2;
    glEnable(GL_TEXTURE_2D);
    glBindTexture(GL_TEXTURE_2D, wallTex);
...
void drawAdvertisements() {
    ...
    int adCount = 8;
    float r = 18.05f, y = 3.5f, charAngleStep = 0.055f;
    GLfloat mat[16];
    ...
    for (int i = 0; i < adCount; i++) {
        ...
        for (int c = 0; c < len; c++) {
            ...
            if (cos(a) * (camX - x) + sin(a) * (camZ - z) < 0)
                continue;
            glRasterPos3f(x, y, z);
    ...
    for (int t = 0; t < 2; t++) {
        ...
        for (int col = 0; col < T.cols; col++) {
            ...
            for (int row = 0; row < T.rows; row++) {
                ...
                glPopMatrix();
                glPushMatrix();
                glTranslatef(0, s * 1, 0);
                glScalef(s * 0.8f, s * 1.0f, s * 0.35f);
                glutSolidCube(1);
                glPopMatrix();
                glPopMatrix();
            }
...
static void drawDrinkCart() {
    ...
    glPushMatrix();
    glTranslatef(-0.17f, 0.26f, 0);
    glScalef(0.01f, 0.20f, 0.03f);
    glutSolidCube(1);
    glPopMatrix();
    glPushMatrix();
    glTranslatef(-0.17f, 0.26f, 0);
    glScalef(0.01f, 0.03f, 0.16f);
    glutSolidCube(1);
    glPopMatrix();
    // wheels
...
void drawOnFieldScene() {
    ...
    // Bench
    {
        ...
        for (int s = 0; s < 3; s++) {
            ...
            glScalef(0.11f, 0.15f, 0.08f);
            glutSolidCube(1);
            glPopMatrix();
            glColor3f(SKIN, SKING, SKINB);
            glPushMatrix();
            glTranslatef(0, 0.23f, 0);
            glutSolidSphere(0.052f, 7, 5);
            glPopMatrix();
            glPopMatrix();
        }
        glPopMatrix();
...
void drawLightCones() {
    ...
    for (int p = 0; p < 6; p++) {
        ...
        glColor4f(1, 1, 0.85f, 0.12f);
        glVertex3f(lx, ly, lz);
        for (int k = 0; k <= coneSegs; k++) {
            float a = k * 2 * PI / coneSegs;
            glColor4f(1, 1, 0.8f, 0);
            glVertex3f(gx + coneRadius * cos(a), gy, gz + coneRadius * sin(a));
        }
        glEnd();
        glBegin(GL_TRIANGLE_FAN);
        glColor4f(1, 1, 0.9f, 0.18f);
        glVertex3f(lx, ly, lz);
        for (int k = 0; k <= coneSegs; k++) {
            float a = k * 2 * PI / coneSegs;
            glColor4f(1, 1, 0.85f, 0);
            glVertex3f(gx + coneRadius * 0.4f * cos(a), gy, gz + coneRadius * 0.4f * sin(a));
        }
        glEnd();
        glBegin(GL_TRIANGLE_FAN);
        ...
        for (int k = 0; k <= 24; k++) {
            float a = k * 2 * PI / 24;
            glColor4f(1, 1, 0.8f, 0);
            glVertex3f(lx + 1.8f * cos(a), ly + 1.8f * sin(a), lz);
        }
        glEnd();
    }
    glDepthMask(GL_TRUE);
...
void display() {
    ...
    for (int i = 0; i < 360; i++) {
        ...
        glBegin(GL_QUADS);
        glVertex3f(25 * cos(t1), 8, 25 * sin(t1));
        glVertex3f(25 * cos(t2), 8, 25 * sin(t2));
        glVertex3f(18 * cos(t2), 6, 18 * sin(t2));
        glVertex3f(18 * cos(t1), 6, 18 * sin(t1));
        glEnd();
    }
    glColor3f(1, 1, 1);
